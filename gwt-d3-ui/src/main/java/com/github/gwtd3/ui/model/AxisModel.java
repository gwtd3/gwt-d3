 ...
 * 
 * <p>
 * The axis is defined by a {@link Scale} function used to convert arbitrary
 * domain values into pixel positions.
 * <p>
 * The mapping between values and positions are defined by two set of range:
 * <ul>
 * <li><b>the visible domain range</b>: the part of the domain values that will
 * be displayed on the axis. The range of domain values can be modified using
 * the {@link #setVisibleDomain(double, double)} methods.
 * <p>
 * 
 * <li><b>the pixel range</b>: the range of pixels corresponding to the domain
 * values. You should never direclty modify the pixel range, since this is
 * usually done automatically by the renderer using the model.
 * </ul>
 * <p>
 * For instance, setting a visible domain range of [20 .. 35.5] to a pixel range
 * of [100..500] create an axis of 400 pixels long and map the value 20 on the
 * pixel 100, the value 35.5 on the pixel 500, and all intermediate values on
 * intermediate pixels.
 * <p>
 * 
...
public class AxisModel<S extends Scale<S>> implements RangeChangeHasHandlers {
	...
	/**
	 * Set the range of domain values currently displayed on the axis. Do not
	 * send events.
	 * 
	 * @param lower
	 * @param upper
	 ...
	 * <p>
	 * You should not change this range since this is done by components using
	 * the model.
	 * 
	 * 
	 * @param start
