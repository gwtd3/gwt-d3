...
public class Zoom extends JavaScriptObject implements IsFunction {
	...
	public static enum ZoomEventType {
        /**
         * Registers the specified listener to receive events of the specified type from 
         * the zoom behavior. Currently, only the "zoom" event is supported. 
         */
		zoom
	}
     ...
     * If not specified, returns the current y-scale, which defaults to null. 
     * If the scale's domain or range is modified programmatically, this function should be called again.
     * @param o
     * @return Zoom object
     */
