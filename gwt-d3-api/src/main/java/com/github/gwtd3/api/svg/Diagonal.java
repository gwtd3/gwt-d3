 ...
 * a cubic
 * Bezier connecting the source and target points; the tangents are specified to
 * produce smooth fan-in and fan-out when
...
public class Diagonal
     ...
     * layouts, including {@link TreeLayout},
     * partition and cluster. For example, to produce a radial diagonal see the
     * <a
     * href=
     ...
     * constant object. The default accessor is a
     ...
     * a JSO with a source attribute and
     * returns it. The source-accessor is invoked in the same manner as other
     * value functions in D3. The function is
     * passed two arguments, the current datum (d) and the current index (i). It
     * is also possible to specify the
     * source-accessor as a constant {@link #source(Coords)} rather than a
     * function.
     * 
     ...
     * a JSO with a target attribute and
     * returns it. The target-accessor is invoked in the same manner as other
     * value functions in D3. The function is
     * passed two arguments, the current datum (d) and the current index (i). It
     * is also possible to specify the
     * target-accessor as a constant {@link #target(Coords)} rather than a
     * function.
     * 
